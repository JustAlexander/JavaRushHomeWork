    ...
    public static void sort(List<Stock> list) {
        Collections.sort(list, new Comparator<Stock>() {
            public int compare(Stock stock1, Stock stock2) {
                int comapreResult;
                comapreResult = stock1.get("name").toString().compareTo(stock2.get("name").toString());
                if (comapreResult != 0) return comapreResult;
                else {
                    SimpleDateFormat sdfWithoutTime = new SimpleDateFormat("yyyyMMdd");
                    Date d1 = (Date) stock1.get("date");
                    Date d2 = (Date) stock2.get("date");
                    comapreResult = sdfWithoutTime.format(d2).compareTo(sdfWithoutTime.format(d1));
                    if (comapreResult != 0) return comapreResult;
                    else {
